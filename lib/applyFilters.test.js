...
const { prepQuery } = require("./hor/getManyHOR");
...
describe("getManyHor", () => {
  test("simple", () => {
    let dbHandle = sq.from`test_table`;
    const where = { bird: { eq: "fly" } };
    const results = prepQuery(sq, dbHandle, null, where);
  ...
  test("Bug where if exists was only clause it failed", () => {
    let dbHandle = sq.from`test_table`;
    const where = {
      bing: {
        name: {
    ...
    const existsFilters = {
      bing: {
        tableName: "bing",
        joinCondition: "bing.kf_id = test_table.id"
      }
    };
    const results = prepQuery(sq, dbHandle, existsFilters, where);
    expect(results.query).toEqual({
      text:
        "select * from test_table where (EXISTS ((select $1 from bing where bing.kf_id = test_table.id and name ILIKE $2 limit $3)))",
      args: [1, "%foo%", 1],
      type: "select"
    });
  });
